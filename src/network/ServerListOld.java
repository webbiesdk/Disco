...
public class ServerListOld {
    public static void main(String[] args) {
        ...
        list.addAddedIpObserver(new IPChangedObserver() {
            ...
            public void ipChanged(InetAddress address) {
                System.out.println("Added: " + address.toString().substring(1));
                System.out.println("Current list: " + list.getList());
            }
        });
        list.addDeletedIpListener(new IPChangedObserver() {
            @Override
            public void ipChanged(InetAddress address) {
                System.out.println("Deleted: " + address.toString().substring(1));
                System.out.println("Current list: " + list.getList());
            }
        });
        list.start();
    ...
    // The listeners.
    private ArrayList<IPChangedObserver> addedIpListeners;
    private ArrayList<IPChangedObserver> deletedIpListeners;
    // The ports
    ...
    public ServerListOld(final int announcePort, final int listenPort) {
        ...
        addedIpListeners = new ArrayList<>();
        deletedIpListeners = new ArrayList<>();
    ...
    public synchronized void addAddedIpObserver(IPChangedObserver listener) {
        addedIpListeners.add(listener);
    }

    public synchronized void addDeletedIpListener(IPChangedObserver listener) {
        deletedIpListeners.add(listener);
    }

    public synchronized void removeAddedIpListener(IPChangedObserver listener) {
        if (addedIpListeners.contains(listener)) {
            addedIpListeners.remove(listener);
        }
    }

    public synchronized void removeDeletedIpListener(IPChangedObserver listener) {
    ...
    private synchronized void runAddedIp(final InetAddress ip) {
        ...
        final ArrayList<IPChangedObserver> localList = (ArrayList<IPChangedObserver>) addedIpListeners.clone(); // Avoiding concurrentModificationException.
        new Thread(new Runnable() {
            @Override
            public void run() {
                for (IPChangedObserver action : localList) {
                    action.ipChanged(ip);
                }
            }
        }).start();
    ...
    private synchronized void runDeletedIp(final InetAddress ip) {
        ...
        final int eventCountLocal = eventCount;
        @SuppressWarnings("unchecked")
        final ArrayList<IPChangedObserver> localList = (ArrayList<IPChangedObserver>) deletedIpListeners.clone(); // Avoiding concurrentModificationException.
        new Thread(new Runnable() {
            @Override
            public void run() {
                for (IPChangedObserver action : localList) {
